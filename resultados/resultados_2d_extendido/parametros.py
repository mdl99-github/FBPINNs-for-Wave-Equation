...
# Script para encontrar los pesos y medias de los pulsos gaussianos para la reconstrucción de una imagen.
...
class GaussianPulseOptimizer:
    ...
    def model(self, x):
        pulses = torch.stack([self.weights[i] * self.gaussian_kernel(x, self.means[i]) 
...
img = Image.open('tejido_pequeno222.png') 
