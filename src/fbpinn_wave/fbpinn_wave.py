...
def exp_decay(length, scale):
    ...
    return np.exp(-scale*t**2)

def extend_sinogram(s, r, c, r_new, dt, t_end, t0=0, margin=0.2, n_resample=0, smooth=False):
    '''
      Extiende el sinograma para sensores en un radio mayor al original y para un tiempo mayor. Asume que la onda ya pasó por los sensores y para el tiempo final original su valor es cercano a 0. Recibe:

      's': Sinograma a extender.

      'r': Distancia de los sensores al origen.

      'c': Velocidad de propagación de la onda. Se supone constante.

      'r_new': Distancia de los sensores al origen para el nuevo sinograma extendido,

      'dt': Resolución temporal.

      't_end': Nuevo tiempo final de simulación.

      't0': Tiempo de inicio de simulación, el de 's'. Por default vale 0.

      'margin': Proporción extra desde que pasa el centro del pulso por los sensores que debe mantenerse visible entodo momento. Por default vale 0.2.

      'n_resample': Número de muestras temporales del sinograma final. En caso de ser necesario, se hace un resampling. Por default vale 0, que significaría que no se quiere hacer resampling.

      'smooth': Si se quiere o no suavizar la transición entre el sinograma original y el padding con 0s. Por default vale False y no se hace nada.


      Devuelve el sinograma extendido y el nnuevo eje de tiempo.

    '''
    t_end0 = s.shape[1]*dt + t0
    ti_add = int((t_end - t_end0)/dt)
    if ti_add > 0:
        s_new = np.pad(s, ((0, 0), (0, ti_add)))
    else:
        s_new  = s
    ...
    assert abs(shift) <= abs(max_shift), "Corrimiento inválido: El pulso debe estar visible en todo momento."

    s_new = np.roll(s_new, shift, axis=1)*fact

    if n_resample != 0:
        s_resampled = np.zeros((s.shape[0], n_resample))
        for sensor in range(s_new.shape[0]):
            s_resampled[sensor,:] = resample(s_new[sensor,:], n_resample)
        s_new = s_resampled
    t = np.linspace(t0, t_end, s_new.shape[1])
    
    return s_new, t

def metricas_imagen(u1, u2):
    '''
      Imprime por pantalla el error cuadrático medio, su raíz cuadrada, la relación señal a ruido pico, el índice de similitud estructural y el coeficiente de correlación de Pearson. Recibe:

      'u1': La primera imagen.

      'u2': La segunda imagen.
